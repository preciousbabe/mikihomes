...
document.getElementById("toggleViewBtn").addEventListener("click", function(event) {
    ...
    let hiddenCards = document.querySelectorAll(".menu__card.hidden, .menu__card.extra");
    
    hiddenCards.forEach(card => {
        if (card.classList.contains("hidden")) {
            card.classList.remove("hidden");  // Show properties
            card.classList.add("extra");  // Mark as extra for toggling
        } else {
            card.classList.add("hidden"); // Hide properties
            card.classList.remove("extra");
        }
    });

    // Toggle button text based on visibility
    if (document.querySelector(".menu__card.hidden")) {
        this.innerHTML = 'View More <i class="ri-arrow-right-long-line"></i>';
    } else {
        this.innerHTML = 'View Less <i class="ri-arrow-up-line"></i>';
    }
});
